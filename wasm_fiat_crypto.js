...
(u=>async a=>{"use strict";let{src:r,generated:C}=a;function
F(a){const
...
g=globalThis?.process?.versions?.node,o=g?F(r):B(r);let
...
f=g&&require("fs");let
e=f?.constants,H=f?[e.RDONLY,e.O_WRONLY,e.O_APPEND,e.O_CREAT,e.O_TRUNC,e.O_EXCL,e.O_NONBLOCK]:[];var
b={map:new
WeakMap(),set:new
...
E(a,b){for(var
c=0;c<b.length;c++)a=D(a,b.charCodeAt(c));return a^b.length}let
...
c
in
a)if(a.hasOwnProperty(c))b(c)},array_length:a=>a.length,array_get:(a,b)=>a[b],array_set:(a,b,c)=>a[b]=c,read_string:a=>j.decode(new
Uint8Array(h,0,a)),read_string_stream:(a,b)=>j.decode(new
Uint8Array(h,0,a),{stream:b}),append_string:(a,b)=>a+b,write_string:a=>{var
c=0,b=a.length;while(1){let{read:d,written:e}=A.encodeInto(a.slice(c),I);b-=d;if(!b)return e;x(e);c+=d}},ta_create:(a,b)=>new
...
Date(Date.UTC(b.getUTCFullYear(),0,1)).getTime(),d=Math.floor((c-e)/86400000);return m(b.getUTCSeconds(),b.getUTCMinutes(),b.getUTCHours(),b.getUTCDate(),b.getUTCMonth(),b.getUTCFullYear()-1900,b.getUTCDay(),d,false)},localtime:a=>{var
b=new
Date(a*1000),c=b.getTime(),f=new
Date(b.getFullYear(),0,1).getTime(),d=Math.floor((c-f)/86400000),e=new
Date(b.getFullYear(),0,1),g=new
Date(b.getFullYear(),6,1),h=Math.max(e.getTimezoneOffset(),g.getTimezoneOffset());return m(b.getSeconds(),b.getMinutes(),b.getHours(),b.getDate(),b.getMonth(),b.getFullYear()-1900,b.getDay(),d,b.getTimezoneOffset()<h)},mktime:(a,b,c,d,e,f)=>new
Date(a,b,c,d,e,f).getTime(),random_seed:()=>crypto.getRandomValues(new
Int32Array(12)),open:(a,d,c)=>f.openSync(a,H.reduce((a,b,c)=>d&1<<c?a|b:a,0),c),close:a=>f.closeSync(a),write:(a,b,c,d,e)=>f?f.writeSync(a,b,c,d,e==null?e:Number(e)):(console[a==2?"error":"log"](typeof
b=="string"?b:j.decode(b.slice(c,c+d))),d),read:(a,b,c,d,e)=>f.readSync(a,b,c,d,e),file_size:a=>f.fstatSync(a,{bigint:true}).size,register_channel:J,unregister_channel:K,channel_list:z,exit:a=>g&&d.exit(a),argv:()=>g?d.argv.slice(1):["a.out"],getenv:a=>g?d.env[a]:null,system:a=>{var
b=require("child_process").spawnSync(a,{shell:true,stdio:"inherit"});if(b.error)throw b.error;return b.signal?255:b.status},time:()=>performance.now(),getcwd:()=>g?d.cwd():"/static",chdir:a=>d.chdir(a),mkdir:(a,b)=>f.mkdirSync(a,b),unlink:a=>f.unlinkSync(a),readdir:a=>f.readdirSync(a),file_exists:a=>+f.existsSync(a),rename:(a,b)=>f.renameSync(a,b),throw:a=>{throw a},start_fiber:a=>s(a),suspend_fiber:l({parameters:["externref","funcref","eqref"],results:["eqref"]},(c,b)=>new
...
WeakMap,map_new:()=>new
Map,map_get:(a,b)=>{var
c=a.get(b);return c==undefined?null:c},map_set:(a,b,c)=>a.set(b,c),map_delete:(a,b)=>a.delete(b),log:a=>console.log("ZZZZZ",a)},k={test:a=>+(typeof
a==="string"),compare:(a,b)=>a<b?-1:+(a>b),hash:E,decodeStringFromUTF8Array:()=>"",encodeStringToUTF8Array:()=>0};const
p=Object.assign({Math:G,bindings:v,js:u,"wasm:js-string":k,"wasm:text-decoder":k,"wasm:text-encoder":k,env:{}},C),q={builtins:["js-string","text-decoder","text-encoder"]},L=g?await
WebAssembly.instantiate(await
o,p,q):await
WebAssembly.instantiateStreaming(o,p,q);var{caml_callback:c,caml_alloc_tm:m,caml_start_fiber:y,caml_handle_uncaught_exception:n,caml_buffer:w,caml_extract_string:x,_initialize:i}=L.instance.exports,h=w?.buffer,I=h&&new
Uint8Array(h,0,h.length);s=l({parameters:["eqref"],results:["externref"]},y,{promising:"first"});var
i=l({parameters:[],results:["externref"]},i,{promising:"first"}),d=globalThis.process;if(d&&d.on)d.on("uncaughtException",(a,b)=>n(a));else if(globalThis.addEventListener)globalThis.addEventListener("error",a=>a.error&&n(a.error));await
i()})(function(a){"use strict";return{}}(globalThis))({"generated":(a=>{var

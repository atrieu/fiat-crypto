...
Section MLDSA.
  ...
  Lemma mldsa_zeta_m_ok:
    F.pow zeta (N.of_nat (Nat.pow 2 m)) = F.of_Z _ (-1).
  Proof.
    assert (Nat.pow 2 m = 256)%nat as -> by reflexivity.
  ...
  Definition mldsa_zetas := List.map (fun k => nth_default 0%F (List.map (F.of_Z q) mldsa_zetas_256) k) (@GallinaNTT.zeta_powers m (Nat.pow 2 m) m).

  Lemma mldsa_zetas_correct:
    mldsa_zetas = List.map (fun k => F.pow zeta (N.of_nat k)) (@GallinaNTT.zeta_powers m (Nat.pow 2 m) m).
  Proof.
  ...
  Lemma mldsa_c_correct:
    mldsa_c = F.inv (F.pow (1 + 1)%F (N.of_nat (Nat.min m n))).
  Proof.
    apply ModularArithmeticTheorems.F.eq_to_Z_iff.
    reflexivity.
  Qed.

  Definition mldsa_ntt := @br2_ntt 64 (Naive.word _) q n m mldsa_zetas word_of_F add sub mul.
  Definition mldsa_inverse_ntt := @br2_ntt_inverse 64 (Naive.word _) q n m mldsa_c mldsa_zetas word_of_F add sub mul.
  ...
  Lemma mldsa_ntt_ok:
    @spec_of_ntt _ _ _ _ _ _ "mldsa_ntt" q n m mldsa_zetas (feval (modulus_small:=q_small) (modulus_prime:=mldsa_prime_q)) (map.of_list mldsa_funcs).
  Proof.
    eapply (br2_ntt_ok mldsa_c mldsa_zetas mldsa_c_correct).
    2-4: cbn; Lia.lia.
    - eapply feval_ok.
    - reflexivity.
  ...
  Lemma mldsa_inverse_ntt_ok:
    @spec_of_ntt_inverse _ _ _ _ _ _ "mldsa_inverse_ntt" q n m mldsa_zetas (feval (modulus_small:=q_small) (modulus_prime:=mldsa_prime_q)) (map.of_list mldsa_funcs).
  Proof.
    eapply (br2_ntt_inverse_ok mldsa_c mldsa_zetas mldsa_c_correct).
    2-4: cbn; Lia.lia.
    - eapply feval_ok.
    - reflexivity.
    - apply mldsa_felem_mul_ok.
  ...
  Compute (ToCString.c_module mldsa_funcs).
End MLDSA.

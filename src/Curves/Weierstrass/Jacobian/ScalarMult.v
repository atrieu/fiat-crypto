...
Module ScalarMult.
  ...
  Section Ladders.
    ...
    Local Notation zacau := (@Jacobian.zacau F Feq Fzero Fone Fopp Fadd Fsub Fmul Finv Fdiv a b field Feq_dec).
    ...
    Program Definition dblu_co_z_points (P : point) (HPaff : z_of P = 1) : co_z_points :=
      dblu P HPaff.
    Next Obligation. Proof. t. Qed.
    ...
    Program Definition zacau_co_z_points (AB : co_z_points) (C : F) (H' : let '(P, Q) := proj1_sig AB in let x1 := x_of P in let x2 := x_of Q in C = (x1 - x2)^2) : co_z_points * F :=
      zacau (fst (proj1_sig AB)) (snd (proj1_sig AB)) C _ _.
    Next Obligation. Proof. generalize (proj2_sig AB); rewrite (surjective_pairing _) at 1. auto. Qed.
    Next Obligation. Proof. rewrite (surjective_pairing _) in H'. assumption. Qed.
    Next Obligation. Proof. destruct AB as ((P & Q) & ?). destruct P as (((X1 & Y1) & Z1) & ?). destruct Q as (((X2 & Y2) & Z2) & ?). cbv. reflexivity. Qed.
    ...
    Section Montgomery.
      (* Scalar Multiplication on Weierstraß Elliptic Curves from Co-Z Arithmetic *)
      (* Goundar, Joye, Miyaji, Rivain, Vanelli *)
      (* Adapted version of Algorithm 15 with full coordinates *)
      Definition mont_ladder_full (scalarbitsz : Z) (testbit : Z -> bool) (P : point) (HPaff : z_of P = 1) : point :=
        let R1R0 := dblu_co_z_points P HPaff in
        let C := let '(R1, R0) := proj1_sig R1R0 in
                 (x_of R0 - x_of R1)^2 in



        let '(R1R0, _, _) :=
          (@while (co_z_points*F*Z) (fun '(_, i) => Z.ltb 0 i)
             (fun '(R1R0, C, i) =>
                let b := testbit i in
                let R1R0 := cswap_co_z_points b R1R0 in
                let R1R0 := cswap_co_z_points b (zacau_co_z_points R1R0 C) in
                let i := Z.pred i in
                (R1R0, C, i))
             (Z.to_nat scalarbitsz) (* bound on loop iterations *)
             (R1R0, C, (scalarbits - 2)%Z)) in
        snd (proj1_sig R1R0).
